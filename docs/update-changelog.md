...
Purpose: curate user-facing changes since the last release tag and record them in `CHANGELOG.md` (Unreleased section) for this repo. Derived from the `/update-changelog` prompt, the macOS release notes guide, and app notes.
...
- Target file: the repo’s `CHANGELOG.md` (keep entries app-specific).
